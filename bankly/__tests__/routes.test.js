...
beforeEach(async function() {
	...
	const badToken = jwt.sign({ username: 'badtoken', admin: 'true' }, 'badsecret');
	tokens.bad = badToken;
});
...
describe('POST /auth/login', function() {
	...
	// TESTS BUG #5
	test('should not allow an incorrect username/password combo to log in', async function() {
		const response = await request(app).post('/auth/login').send({
			username : 'u2',
			password : 'pwd1'
		});
...
describe('GET /users', function() {
	...
	// TESTS BUG #6: INVALID TOKEN SHOULD THROW AN ERROR
